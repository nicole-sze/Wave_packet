...
def wavepacket(t, k, v0):
	...
	# Setting values for space and time steps
	dt = 0.001
	dx = 0.15
	a = 1
...
inputs = list(map(float, input('Enter normalised Gaussian width (a) and wave number (k), barrier potential (v0): ').split()))

# 2D plot with 9 different time periods
for m in range(len(T_values)):
	grid, psi_n1, p, b, dx = wavepacket(T_values[m], inputs[0], inputs[1], inputs[2])
