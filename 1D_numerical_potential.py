...
# 2D plot
for m in range(9):
    grid, psi_n1 = wavepacket(T_values[m], inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7])
    plt.subplot(3, 3, m+1)
    plt.axhline(y=inputs[7], xmin=(inputs[6]-inputs[4]/2), xmax=(inputs[6]+inputs[4]/2))
...
plt.tight_layout()
plt.savefig('1Dnumerical_potential.pdf')
plt.show()